...
# config.toml tokens so beryl follows the rest of the rice, light themes and
# all. Colors are #AARRGGBB — the chrome stays translucent so Hyprland's blur
# shows through it; the page itself sits on the opaque viewBg.
_BASE = {
    "bg":        "#66161a24",   # window glass
    "card":      "#ee161a24",   # near-opaque overlays (completion, cheat sheet)
    ...
    "sel":       "#4d7aa2f7",   # active tab / completion selection wash
    "selText":   "#ffc8ccd8",
    ...
    "accentSoft":"#337aa2f7",
    "warn":      "#ffe0af68",
    "onAccent":  "#ff11121a",   # glyphs on accent fills (mode pill)

    "viewBg":    "#ff11121a",   # opaque page background (kills white flash)
...
def _active_theme_dir():
    ...
    resolution the quickshell loaders, frostify, mica and vellum use.
    BERYL_THEME (a name or a path) overrides it for headless testing. None if
    nothing resolves."""
    override = os.environ.get("BERYL_THEME", "").strip()
    if override:
...
class ThemeManager(QObject):
    ...
    config.toml and beryl.qml, so switching themes or editing one re-skins
    beryl while it runs. themeChanged fires after each rebuild; main.py
    re-sets the Theme context property, which re-evaluates every Theme.*
    binding in the QML, and Main.qml reloads the chrome slot (the theme's
    optional beryl.qml — same slot grammar as frostify/mica/vellum)."""
    themeChanged = Signal()
    ...
    # -- chrome slot (the theme's optional beryl.qml — same slot grammar as
    # frostify/mica/vellum: pal/host injection, backdrop/overlay Components) --
    @Property(str, notify=themeChanged)
    def source(self):
        """file:// URL of the theme's beryl.qml with a ?v= nonce (hot reload),
    ...
    def _refresh(self, first=False):
        ...
        qml = (d / "beryl.qml") if d else None
        qml_stat = None
        qml_text = ""
        ...
        if qml_stat is not None:
            # same handshake as frostify / mica / the shell's loaders: grep the
        ...
        print(f"[theme] {self._dir.name if self._dir else '(defaults)'}"
              f"  chrome={'beryl.qml' if self._source else 'none'}", flush=True)
        if not first:
    ...
    def _rewatch(self, d):
        ...
        if d is not None:
            want += [d, d / "config.toml", d / "beryl.qml"]
